...
public class RegionForm extends TabbedForm implements IEntityForm {
            ...
            worker = new SwingWorker<ArrayList<Region>, Void>() {
                ...
                protected ArrayList<Region> doInBackground() throws Exception {
                    ...
                    if (type.equals("install")) {
                        Notifications.getInstance().show(Notifications.Type.INFO, "Region data loading...");
                    }
                    ...
                    for(Region r : regionList) {
                        Object[] add = {false, r.regionID(), r.regionName()};
                        model.addRow(add);
                    }
                ...
                protected void done() {
                    try {
                        ArrayList<Region> regionList = get();
            ...
            int counter = 0;
                ...
                protected Void doInBackground() throws Exception {
                    ...
                    for(Boolean b : selectedValueList) {
                        i++;
                        if (b) {
                            Region region = new Region((int) getSelectedValue(model, i-1, 1));
                            databaseOperation.deleteRegionFromDatabase(region);
                            counter++;
                        }   
                    }
                    resetData(model);
                ...
                protected void done() {
                    try {
                        if (counter == 1) {
                            Notifications.getInstance().show(Notifications.Type.ERROR, "Region has been deleted");
                        }
                        else {
                            Notifications.getInstance().show(Notifications.Type.ERROR, "Regions has been deleted");
                        }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("Region Manager");
    ...
    private void cmdNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdNewActionPerformed
        ...
        GlassPanePopup.showPopup(new SimplePopupBorder(createRegion, "Create Region", actions, (pc, i) -> {            
            if (i == 1) {
                try {                                    
                    SwingWorker<Void, Void> worker;
                    worker = new SwingWorker<Void, Void>() {
                        ...
                        @Override
                        protected Void doInBackground() throws Exception {
                            String regionName = createRegion.getTxtName().getText();
                            DatabaseOperation databaseOperation = new DatabaseOperation();
                            Region region = new Region(regionName, databaseOperation.increaseID("region"));
                            databaseOperation.addRegionToDatabase(region);
                            resetData(model);
                            Notifications.getInstance().show(Notifications.Type.INFO, "Region data refreshing...");
                            return null;
                        }
                        ...
                        protected void done() {
                            try {
                                insertData("update");
                                pc.closePopup();
                                Notifications.getInstance().show(Notifications.Type.SUCCESS, "Region has been created");
