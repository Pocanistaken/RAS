...
public class TableForm extends TabbedForm implements IEntityForm {
    ...
    private void initComponents() {
        ...
        table.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "SELECT", "ID", "NAME", "CHAIR AMOUNT", "PHONE", "STATUS", "REGION"
            }
        ) {
    ...
    private void cmdNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cmdNewActionPerformed
        ...
        // Add anti bug checkers - Pocan
            ...
            GlassPanePopup.showPopup(new SimplePopupBorder(createTable, "Create Table", actions, (pc, i) -> {            
                if (i == 1) {
                    // save
                    try {
                        ...
                        worker = new SwingWorker<Void, Void>() {
                            ...
                            protected Void doInBackground() throws Exception {
                                ...
                                String tableName = createTable.getTxtName().getText();
                                String chairAmount = createTable.getTxtChairAmount().getText();
                                Region selectedRegion = (Region) createTable.getComboRegion().getSelectedItem();

                                System.out.println("DEBUG -> " + selectedRegion.regionName() + "-" + selectedRegion.regionID());

                                DatabaseOperation databaseOperation = new DatabaseOperation();


                                Table table = new Table(tableName, Integer.valueOf(chairAmount), "EMPTY", TableStatus.EMPTY.name(), selectedRegion, databaseOperation.increaseID("table"));

                                databaseOperation.addTableToDatabase(table);
                                resetData(model);
                                return null;
                            }
                            ...
                            protected void done() {
                                try {
                                    insertData("update");
                                    pc.closePopup();
                                    Notifications.getInstance().show(Notifications.Type.SUCCESS, "Table has been created");
    ...
    public void insertData(String type) {
        ...
        try {                                    
            SwingWorker<ArrayList<Table>, Void> worker;
            worker = new SwingWorker<ArrayList<Table>, Void>() {

                @Override
                protected ArrayList<Table> doInBackground() throws Exception {
                    
                    
                    if (type.equals("install")) {
                        Notifications.getInstance().show(Notifications.Type.INFO, "Table data loading...");
                    }
                    
                    DatabaseOperation databaseOperation = new DatabaseOperation();
                    ArrayList<Table> tableList = databaseOperation.getAllTables();
                    
                    int i = 0;
                    for(Table t : tableList) {
                        Object[] add = {false, t.getTableID(), t.getTableName(), t.getTableChairAmount(), t.getTableOwnerPhoneNumber(), t.getTableStatus(), t.getTableRegion().regionName()};
                        model.addRow(add);
                    }
            ...
            int counter = 0;
                ...
                protected Void doInBackground() throws Exception {
                    ...
                    for(Boolean b : selectedValueList) {
                        i++;
                        if (b) {
                            Table table = new Table((int) getSelectedValue(model, i-1, 1));
                            databaseOperation.deleteTableFromDatabase(table);
                            counter++;
                        }   
                    }
                    resetData(model);
                ...
                protected void done() {
                    try {
                        if (counter == 1) {
                            Notifications.getInstance().show(Notifications.Type.ERROR, "Table has been deleted");
                        }
                        else {
                            Notifications.getInstance().show(Notifications.Type.ERROR, "Tables has been deleted");
                        }
